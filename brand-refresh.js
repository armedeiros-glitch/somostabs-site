document.addEventListener('DOMContentLoaded',()=>{
  ...
  style.textContent=`
    ...
    .hero-product-card img{display:block;width:100%;height:auto;max-height:620px;object-fit:contain;image-rendering:auto;filter:drop-shadow(0 28px 50px rgba(0,0,0,.55))}
    ...
    @media(max-width:900px){
      .tabs-logo small{display:none}.brand{min-width:auto}
      .hero-product-card{max-width:480px;margin:auto}.hero-visual{min-height:480px}
      .pack-grid{grid-template-columns:1fr}.pack-card{min-height:460px}
    }
    @media(max-width:560px){
      .hero-visual{min-height:410px}.hero-product-card img{max-height:430px}
      .hero-product-tag{font-size:.57rem;left:10px;bottom:10px}
      .pack-card{min-height:400px;padding:18px}.pack-card img{max-height:430px}
    }
  `;
  ...
  if(brand){
    brand.innerHTML='<span class="tabs-logo" aria-label="ALT TABS, sua mente no modo certo"><span class="tabs-logo-alt">ALT</span><span class="tabs-logo-speed">≡</span><span class="tabs-logo-tabs">TABS</span><small>Sua mente no modo certo.</small></span>';
  }

  const hero=document.querySelector('.hero-visual');
  if(hero){
    hero.innerHTML=`<div class="hero-product-card">
      <img src="${pack30}" alt="ALT TABS Smart Gum, conceito de embalagem pouch 30 gums" width="1122" height="1402" decoding="async" fetchpriority="high">
      <div class="hero-product-tag">Conceito de embalagem</div>
    </div>`;
  }
  ...
  if(moments && !document.querySelector('.pack-showcase')){
    ...
    section.innerHTML=`<div class="container">
      <div class="pack-showcase-head">
        <div class="kicker">Identidade em desenvolvimento</div>
        <h2>Smart Gum com cara de ALT TABS.</h2>
        <p>Do pouch para a caixa, a linguagem visual segue o mesmo território: performance, foco, tecnologia e presença. <strong style="color:#fff">Sua mente no modo certo.</strong></p>
      </div>
      <div class="pack-grid">
        <div class="pack-card"><img src="${pack30}" alt="ALT TABS pouch, conceito 30 gums" width="1122" height="1402" loading="lazy"><span>Pouch · conceito 30 gums</span></div>
        <div class="pack-card"><img src="${pack8}" alt="ALT TABS box, conceito 8 gums" width="1122" height="1402" loading="lazy"><span>Box · conceito 8 gums</span></div>
      </div>
    </div>`;
